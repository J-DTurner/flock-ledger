...
function draftDirty(cmd:any,s:any){
  ...
  return ['name','phase','cost','kg','qtyUnknown','lotId','avgKg','minKg','maxKg','sampleN','weights','count','liveKg','dressedKg','revenue','homeKg','note','sacks','packKg'].some(k=>String(s[k]??'')!==String(f[k]??'')) || (s.extraLots&&s.extraLots.length);
}
...
function ActionChooser({onPick,onCancel}:any){
  return <div><div className="modal-heading"><div><span className="eyebrow">ADD</span><h2>Choose an action</h2></div><button type="button" className="icon-btn" aria-label="Close form" onClick={onCancel}><Icon name="close"/></button></div><p className="muted small-text">Named tasks open the matching form. This chooser is only for when the task is not already known.</p><div className="chooser-grid">{CHOOSER.map(([action,label])=><button key={action} type="button" className="chooser-btn" onClick={()=>onPick(action)}>{label}</button>)}</div></div>;
}
class TaskForm extends React.Component<any,any>{
  ...
  endDate=()=>this.state.date||this.props.command.date||M.today();
  ...
  build=()=>{
    ...
    if(cmd.action==='usage'){
      ...
      return rows.map((r:any,i:number)=>({...base,id:i===0?base.id:M.uid(),type:'usage',kg:r.kg,lotId:r.lotId,startDate:s.startDate,date:this.endDate()}));
    }
    if(cmd.action==='weigh'){Object.assign(base,M.serializeWeigh(s.weighMode,{avgKg:s.weighUnit==='g'&&s.avgKg!==''?String(Number(s.avgKg)/1000):s.avgKg,minKg:s.weighUnit==='g'&&s.minKg!==''?String(Number(s.minKg)/1000):s.minKg,maxKg:s.weighUnit==='g'&&s.maxKg!==''?String(Number(s.maxKg)/1000):s.maxKg,sampleN:s.sampleN,weights:s.weighUnit==='g'?s.weights.split(/[\s,;]+/).filter(Boolean).map((w:string)=>String(Number(w)/1000)).join(', '):s.weights}));}
  ...
  save=(ev:any,andAnother=false)=>{ev.preventDefault();try{this.props.onSave(this.build(),{andAnother,command:this.props.command});}catch(err:any){this.setState({error:err.message});}};
  ...
  renderUsage(batch:any){
    ...
    return <>
      ...
      {bal&&<div className="wide live-preview"><div>Balance at this date: <strong>{qty(bal.balanceAtDate)} kg</strong></div><div>Remaining after all recorded usage: <strong>{qty(bal.remainingAfterAll)} kg</strong></div>{Number(s.kg)>0&&selected&&selected.kg&&<div>Recorded stock after this entry: <strong>{qty((selected.kg===null?null:selected.kg-M.lotBalances(batch,s.lotId).usedAll-(this.props.command.event?0:Number(s.kg))))} kg</strong></div>}</div>}
  ...
  renderWeigh(){
    ...
    return <>
      ...
      <Field label="Unit"><select value={s.weighUnit} onChange={e=>this.set('weighUnit',e.target.value)}><option value="kg">Kilograms</option><option value="g">Grams (converted to kg)</option></select></Field>
      ...
      {s.weighMode==='individual'&&<><Field label="Individual weights (kg)" wide hint="Separate birds with commas, spaces, or new lines. A period is the decimal mark."><textarea rows={3} placeholder="0.98, 1.04, 1.10, 0.95" value={s.weights} onChange={e=>this.set('weights',e.target.value)}/></Field>
  ...
  render(){
    ...
    return <form onSubmit={e=>this.save(e,false)}>
      ...
      <div className="entry-context"><span>{bname}</span><span>{dateLabel(s.date||cmd.date)}</span></div>
      {cmd.convert&&<Note>The budget will be replaced by this purchase when you save. Enter the actual kilograms and amount paid.</Note>}
...
class App extends React.Component<any,any>{
  ...
  saveRecords=(records:any[],opts:any)=>{
    ...
    next.settings.ui=ui;
    M.validateState(next);storeWrite(next);
  ...
  deleteRecord=(event:any)=>{if(!window.confirm(`Delete this ${TYPELABEL[event.type].toLowerCase()}? This can change stock and costs.`))return;try{this.mutate((d:any,b:any)=>{b.events=b.events.filter((e:any)=>e.id!==event.id);},'Record deleted');this.setState({lastSave:null});}catch(e:any){this.toast(e.message);}};
  ...
  renderToday(){
    ...
    const latestEstimate=b.events.filter((e:any)=>e.type==='weigh'&&e.method==='estimate'&&e.date&&e.date<=date).sort((a:any,c:any)=>c.date.localeCompare(a.date))[0];
    return <>
      <div className="page-title"><div><span className="eyebrow">TODAY</span><h1>{b.name}</h1><p>{age<0?'Purchase is in the future':`Day ${age} since purchase`} · {b.placementEstimated?'Approximate purchase date':'Purchased'} {dateLabel(b.placementDate)}</p></div><button className="btn secondary small" onClick={()=>this.setState({modal:{kind:'batch',batch:b}})}>Change</button></div>
      <div className="today-date"><Field label="Today · selected date"><input type="date" aria-label="Selected date" value={date} onChange={e=>this.setState({selectedDate:e.target.value})}/></Field></div>
      <div className="quick-actions eight">
        <button onClick={()=>this.openAction({action:'usage',date})}><Icon name="log"/><span>Use feed</span></button>
        ...
        <button onClick={()=>this.openAction({action:'loss',date})}><Icon name="trash"/><span>Loss / removal</span></button>
      ...
      <div className="metrics"><Metric label="Live birds" value={live.confirmed?live.count:'Not counted'} sub={live.confirmed?'Verified count, less subsequent recorded removals':`${live.count} placed-minus-removals; not a verified live count`}/><Metric label="Latest measured sample" value={measured?qty(s.weigh.avgKg)+' kg':'—'} sub={measured?dateLabel(s.weigh.date):'No measured sample yet'}/><Metric label="Latest estimate" value={latestEstimate?qty(latestEstimate.avgKg)+' kg':'—'} sub={latestEstimate?dateLabel(latestEstimate.date):'No estimate recorded'}/><Metric label="Recorded stock" value={s.inventoryIncomplete?'Incomplete':qty(s.inventoryKg)+' kg'} sub={s.inventoryIncomplete?'A purchase quantity is unknown':'Known remaining kilograms'} accent/></div>
      {!!attention.length&&<section className="card">{attention.map((item:any)=><div className="attention-item" key={item.key}><div><strong>{item.title}</strong>{item.detail&&<p>{item.detail}</p>}</div><button type="button" className="text-btn" onClick={()=>{if(item.actionType==='records')this.switchTab('records');else if(item.eventId)this.openAction({action:item.actionType,event:b.events.find((e:any)=>e.id===item.eventId)});else this.openAction({action:item.actionType,date});}}>{item.action}</button></div>)}</section>}
      {this.renderLastSave(b)}
      <section className="card"><SectionTitle title="Today’s entries" sub={dateLabel(date)}/>{!todayEntries.length?<p className="muted small-text">No entry today does not mean a task was not performed.</p>:todayEntries.map((e:any)=><div className="recent-row" key={e.id}><div><Badge>{TYPELABEL[e.type]}</Badge><strong>{e.name||TYPELABEL[e.type]}</strong><small>{recordDescription(e,b)}</small><div className="button-row"><button className="text-btn" onClick={()=>this.openAction({action:e.type,event:e})}>Edit</button><button className="text-btn" onClick={()=>this.deleteRecord(e)}>Undo</button></div></div><span className="date">{dateLabel(e.date)}</span></div>)}</section>
      <div className="two-col"><section className="card"><SectionTitle title="Growth" sub="Kilograms per bird · dated observations"/>{pts.length?<LineChart series={series.filter((x:any)=>x.points.length)} title="Live weight over time" floorZero/>:<Empty title="Your growth chart starts here" text="Add a dated weighing to see progress."/>}{!perf&&<p className="muted small-text">One estimated point is not a growth curve. No growth rate is inferred from it.</p>}</section>
      <section className="card"><SectionTitle title="Spending"/><div className="detail-grid"><div><small>Chicks</small><strong>{money(b.chickCost)}</strong></div><div><small>Feed purchased</small><strong>{money(s.feedPaid)}</strong></div><div><small>Other expenses</small><strong>{money(s.otherPaid)}</strong></div><div><small>Batch cash recorded</small><strong>{money(s.cashPaid)}</strong></div></div><p className="muted small-text">Budgets {money(s.budgets)} and infrastructure {money(this.capital())} stay out of this cash total.</p></section></div>
      <div className="two-col"><section className="card"><SectionTitle title="Feed and production"/><div className="detail-grid"><div><small>Known feed bought</small><strong>{qty(s.knownPurchasedKg)} kg{s.inventoryIncomplete?' + unknown':''}</strong></div><div><small>Recorded feed used</small><strong>{qty(s.recordedUsedKg)} kg</strong></div><div><small>Known stock remaining</small><strong>{qty(s.inventoryKg)} kg</strong></div><div><small>Feed-use cost logged</small><strong>{money(s.usedFeed)}</strong></div></div>{perf&&<Note><strong>{qty(perf.gainG,1)} g/day</strong> observed sample growth ({dateLabel(perf.start)}–{dateLabel(perf.end)}). {perf.fcr!==null?`${qty(perf.fcr)} logged-feed ratio; incomplete usage logs can bias it.`:'No comparable logged-feed ratio is available yet.'}</Note>}</section>
      <section className="card"><SectionTitle title="Harvest results"/><div className="detail-grid"><div><small>Birds harvested</small><strong>{s.harvestedBirds}</strong></div><div><small>Dressed output</small><strong>{qty(s.dressedKg)} kg</strong></div><div><small>Cash received</small><strong>{money(s.revenue)}</strong></div><div><small>Kept for home</small><strong>{qty(s.homeKg)} kg</strong></div></div></section></div>
    </>;
  }
  renderFeed(){
    const b=this.b(),inv=M.feedInventory(b),budgets=b.events.filter((e:any)=>e.type==='budget');
    return <>
      <div className="page-title"><div><span className="eyebrow">FEED</span><h1>Lots on hand</h1><p>Each purchase stays its own lot. Matching names are not combined.</p></div><button className="btn primary" onClick={()=>this.openAction({action:'feed',date:this.state.selectedDate})}><Icon name="plus"/>Buy feed</button></div>
      {this.renderLastSave(b)}
      {!inv.length&&<Empty title="No purchase lots yet" text="Buy feed with a known quantity to create a lot you can use." action={<button className="btn secondary" onClick={()=>this.openAction({action:'feed'})}>Buy feed</button>}/>}
      {inv.map((p:any)=><section className="card lot-card" key={p.id}>
        <h3>{p.name}</h3>
        <p>{dateLabel(p.date)} · {(p.id||'').slice(-6)}</p>
        <div className="detail-grid"><div><small>Bought</small><strong>{qty(p.kg)} kg</strong></div><div><small>Recorded used</small><strong>{qty(p.used)} kg</strong></div><div><small>Recorded remaining</small><strong>{qty(p.remaining)} kg</strong></div><div><small>₱/kg</small><strong>{money(p.price,2)}</strong></div></div>
        {p.kg===null&&<Note tone="warn">Quantity unknown — unavailable for lot-based feed-use logging.</Note>}
        <div className="button-row">
          {p.kg!==null&&<button className="btn secondary small" onClick={()=>this.openAction({action:'usage',lotId:p.id,date:this.state.selectedDate})}>Use feed</button>}
          <button className="btn secondary small" onClick={()=>this.openAction({action:'feed',product:{name:p.name,phase:p.phase,lastKg:p.kg,lastCost:p.cost,lastDate:p.date},date:this.state.selectedDate})}>Buy again</button>
          <button className="text-btn" onClick={()=>this.openAction({action:'feed',event:p})}>View purchase</button>
        </div>
      </section>)}
      <section className="card"><SectionTitle title="Feed budgets" sub="Plans, not cash and not consumption"/>{!budgets.length?<p className="muted small-text">No feed budgets on this batch.</p>:budgets.map((e:any)=><div className="recent-row" key={e.id}><div><strong>{e.name||'Feed budget'}</strong><small>{money(e.cost)} planned · {dateLabel(e.date)}</small><div className="button-row"><button className="text-btn" onClick={()=>this.openAction({action:'feed',convert:e,date:this.state.selectedDate})}>Record purchase</button><button className="text-btn" onClick={()=>this.openAction({action:'feed',convert:e,date:this.state.selectedDate})}>Convert to purchase</button></div></div></div>)}</section>
    </>;
  }
  renderRecords(){const b=this.b(),events=b.events.filter((e:any)=>this.state.filter==='all'||e.type===this.state.filter).sort((a:any,c:any)=>(c.date||'').localeCompare(a.date||''));
    return <><div className="page-title"><div><span className="eyebrow">THE FARM LOGBOOK</span><h1>Every entry, dated.</h1><p>Purchases, consumption, weights, counts and harvests in one place.</p></div><button className="btn primary" onClick={this.openChooser}><Icon name="plus"/>Add record</button></div>
    <section className="card"><div className="filter-row"><label>Show <select aria-label="Filter records" value={this.state.filter} onChange={e=>this.setState({filter:e.target.value})}><option value="all">All records</option>{Object.entries(TYPELABEL).map(([v,l])=><option key={v} value={v}>{l as string}</option>)}</select></label><span className="muted">{events.length} entries</span></div>{!events.length?<Empty title="No entries here yet" text="Add a record for this batch."/>:events.map((e:any)=><article className="record" key={e.id}><div className="record-top"><Badge kind={e.type==='budget'||e.method==='estimate'?'warn':'muted'}>{e.type==='weigh'&&e.method==='estimate'?'Estimated weight':TYPELABEL[e.type]}</Badge><span>{dateLabel(e.date)}</span></div><h3>{e.name||TYPELABEL[e.type]}</h3><p>{recordDescription(e,b)}</p>{e.note&&e.type!=='note'&&<p className="record-note">{e.note}</p>}<div className="record-buttons">{e.type==='budget'&&<button className="text-btn" onClick={()=>this.openAction({action:'feed',convert:e})}>Convert to purchase <Icon name="arrow" size={15}/></button>}<div className="spacer"/><button className="icon-btn" title="Edit record" aria-label={'Edit '+(e.name||TYPELABEL[e.type])} onClick={()=>this.openAction({action:e.type,event:e})}><Icon name="edit" size={17}/></button><button className="icon-btn danger" title="Delete record" aria-label={'Delete '+(e.name||TYPELABEL[e.type])} onClick={()=>this.deleteRecord(e)}><Icon name="trash" size={17}/></button></div></article>)}</section></>;
  }
  ...
  renderForecast(){const b=this.b(),p=this.state.scenarioDraft||b.forecast,f=this.projection(),s=M.summary(b,p.startDate||M.today()),perf=M.recentPerformance(b);const chosen=f.rows.find((r:any)=>r.day===this.state.selectedDay)||f.best;const important=f.rows.filter((r:any)=>[f.startDay,35,38,40,42,45,47,49,52,56,60,70,84,p.endDay,f.best?.day,this.state.selectedDay].includes(r.day));
    ...
    <div className="button-row"><button className="btn secondary small" onClick={()=>{const prop=M.proposeForecastStart(b);if(!prop.startDate){this.toast('Add a weighing first.');return;}this.setState({scenarioDraft:{...p,startDate:prop.startDate,weightKg:prop.weightKg,birds:prop.birds,baselineCost:prop.baselineCost,feedPrice:prop.feedPrice}});this.toast(prop.weightMethod==='estimate'?'Copied the latest estimate; verified count, start-date production cost, and known feed price only.':'Copied the latest weight; only a verified count, start-date production cost, and known feed price were copied.');}}>Use latest records</button><button className="btn secondary small" onClick={()=>this.setState({scenarioDraft:{...p,baselineCost:s.usedOperating}})}>Use logged production costs ({money(s.usedOperating)})</button></div><p className="muted small-text">Logged production costs may be incomplete. Include only chicks, feed already used, and other incurred costs at the projection start. Exclude unused feed, future budgets, and shared infrastructure.</p>
  ...
  renderMore(){return <><div className="page-title"><div><span className="eyebrow">MORE</span><h1>Batches, backup and about</h1><p>Operational tasks stay on Today and Feed. Backup is explicit — this app does not sync.</p></div></div>{this.renderBatches()}{this.renderBackup()}</>;}
  render(){const s=this.state;if(!s.data)return <main className="recovery"><h1>Your saved ledger could not be opened</h1><Note tone="error">{s.loadError}</Note><p>No stored data has been overwritten. Close and reopen the app, or restore a valid JSON backup. Keep a copy of the original data before replacing it.</p><textarea id="recovery-raw" readOnly rows={8} value={(()=>{try{return storeRead()||'';}catch(e){return '';}})()}/><p>Raw saved data is shown above for recovery.</p>{NATIVE&&<button className="btn secondary" onClick={this.chooseBackup}>Choose a JSON backup</button>}<Field label="Paste or selected backup"><textarea rows={6} value={s.importText} onChange={e=>this.setState({importText:e.target.value,importError:''})}/></Field>{s.importError&&<Note tone="error">{s.importError}</Note>}<button className="btn primary" disabled={!s.importText.trim()} onClick={this.importBackup}>Validate and restore</button></main>;
    ...
    <div className="workspace"><header className="topbar"><div className="mobile-brand"><Icon name="flock" size={23}/>Flock Ledger</div><div className="batch-selector"><span className="eyebrow">BATCH</span><select aria-label="Selected batch" value={b.id} onChange={e=>{const v=e.target.value;if(v==='__manage'){this.switchTab('more');return;}if(v==='__new'){this.setState({modal:{kind:'batch'}});return;}try{this.mutate((d:any)=>{d.selectedBatchId=v;});this.setState({scenarioDraft:null,selectedDay:null,filter:'all'});}catch(err:any){this.toast(err.message);}}}>{s.data.batches.map((b:any)=><option key={b.id} value={b.id}>{b.name}{b.status==='closed'?' · closed':''}</option>)}<option value="__manage">Manage batches</option><option value="__new">New batch</option></select></div><div className="top-date">{dateLabel(M.today())}<span><i className="status-dot"/>Saved locally</span></div><button className="header-add" aria-label="Add record" onClick={this.openChooser}><Icon name="plus"/></button></header>
    <main>{s.storageError&&<Note tone="error">{s.storageError}</Note>}{s.tab==='today'?this.renderToday():s.tab==='feed'?this.renderFeed():s.tab==='records'?this.renderRecords():s.tab==='forecast'?this.renderForecast():s.tab==='backup'?this.renderBackup():this.renderMore()}<footer>Flock Ledger · {b.name} · Private, offline records</footer></main></div>
